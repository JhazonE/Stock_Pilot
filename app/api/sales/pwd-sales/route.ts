  ...
  try {
    ...
    const baseWhere = `
      WHERE pt.is_training = 0
        ...
        AND pt.id IN (
          SELECT pos_transaction_id FROM pos_transaction_items WHERE discount_type = 'pwd'
        )
        ${dateClause}
    `;
    ...
    const dataSql = `
      SELECT
        ...
        SUM(CASE WHEN pti.discount_type = 'pwd' THEN pti.discount_amount ELSE 0 END) as discount
      FROM pos_transactions pt
      JOIN sales_transactions st ON pt.sale_id = st.id
      JOIN pos_transaction_items pti ON pti.pos_transaction_id = pt.id
      LEFT JOIN customers c ON st.customer_id = c.id
      ${baseWhere}
      GROUP BY pt.id, DATE(st.created_at), c.name, c.pwd_id, c.pwd_tin
      ORDER BY date DESC
      LIMIT ? OFFSET ?
    `;
    ...
    console.error('Error fetching PWD sales:', error);
    return NextResponse.json(
      { success: false, error: `Failed to fetch PWD sales: ${error.message}` },
      { status: 500 }
    );
  }
}
